...
if exists('s:lastEltLoc')
  exe 'aunmenu '.s:lastEltLoc.'Font.'
  exe 'aunmenu '.s:lastEltLoc.'Dimension.'
  exe 'aunmenu '.s:lastEltLoc.'Counters.'
...
if !exists('s:definedFuncs') " {{{
  ...
  function! Tex_RemoveElementMenus()
	exe 'silent! aunmenu '.s:lastEltLoc.'Font.'
	exe 'silent! aunmenu '.s:lastEltLoc.'Dimension.'
	exe 'silent! aunmenu '.s:lastEltLoc.'Counters.'
	exe 'silent! aunmenu '.s:lastEltLoc.'Various.'
  endfunction
" }}}
...
" }}}
  ...
  function! <SID>Tex_FontDiacritics(name, rhs)
	...
	exe 'amenu '.location." <plug><C-r>=IMAP_PutTextWithMovement('\\".a:rhs
		  \."{<++>}<++>')<CR>"
	exe 'vmenu '.location." \<C-\\>\<C-n>:call VEnclose('\\".a:rhs
		  \."{', '}', '', '')<CR>" 
  endfunction
" }}}
...
if g:tex_menus && g:tex_fontMenus
  ...
  call s:Tex_Fontfont('fontseries{m\ b\ bx\ sb\ c}',
		\ '\fontseries{<++>}<++>')
  call s:Tex_Fontfont('fontshape{n\ it\ sl\ sc\ ui}',
		\ '\fontshape{<++>}<++>')
