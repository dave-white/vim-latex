...
if has('gui_running') && b:tex_menus
  exec 'anoremenu '.b:tex_mainMenuLoc.'.25 '. s:mainMenuNm.'-sepsuite0-  :'

  " menus for compiling / viewing etc.
  exec 'anoremenu '.b:tex_mainMenuLoc.'.30 '.s:mainMenuNm.'&Compile<tab>'.s:mapleader.'ll'.
		\'   :silent! call Tex_RunLaTeX()<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.40 '.s:mainMenuNm.'&View<tab>'.s:mapleader.'lv'.
		\'   :silent! call Tex_ViewLaTeX()<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.50 '.s:mainMenuNm.'&Search<tab>'.s:mapleader.'ls'.
		\'   :silent! call ForwardSearchLaTeX()<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.60 '.s:mainMenuNm.'&Target\ Format<tab>:TTarget'.
		\'   :call SetTeXTarget()<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.70 '.s:mainMenuNm.'&Compiler\ Target<tab>:TCTarget'.
		\'   :call Tex_SetTeXCompilerTarget("Compile", "")<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.80 '.s:mainMenuNm.'&Viewer\ Target<tab>:TVTarget'.
		\'   :call Tex_SetTeXCompilerTarget("View", "")<CR>'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.90 '.s:mainMenuNm.'Set\ &Ignore\ Level<tab>:TCLevel'.
		\'   :TCLevel<CR>'
  exec 'imenu '.b:tex_mainMenuLoc.'.100 '.s:mainMenuNm.'C&omplete\ Ref/Cite'.
		\'   <Plug>Tex_Completion'
  exec 'anoremenu '.b:tex_mainMenuLoc.'.110 '.s:mainMenuNm.'-sepsuite1- :'
  " refreshing folds
  if b:tex_folding
	exec 'anoremenu '.b:tex_mainMenuLoc.'.120 '.s:mainMenuNm.'&Refresh\ Folds<tab>'.s:mapleader.'rf'.
		  \'   :call MakeTexFolds(1)<CR>'
	exec 'anoremenu '.b:tex_mainMenuLoc.'.130 '.s:mainMenuNm.'-sepsuite2- :'
  endif
endif
